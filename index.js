...
module.exports = (function () {
    ...
    return class FileSteward {
         ...
         * @param {String} path
         * @returns {Object}
         * @description Get the information of the file/directory asynchronously.
         */
        async check(path = this.root, options = { relative: true, children: false, recursive: false }) {
         ...
         * @param {String} destPath
         * @param {Object} options
         * @description Copy a file/directory from it's source path to the destination path asynchronously (supports stream mode).
         */
        async copy(srcPath, destPath, options = { stream: true }) {
            ...
            if (_isDirSync(srcPath)) {
                ...
                _assert(await _copyFile(srcPath, destPath, opts), `Failed to copy file "${srcPath}" to "${destPath}".`);
            } else {
                _assert(false, "Other types are not currently supported.");
            }
        }
         ...
         * @param {String} destPath
         * @param {Object} options
         * @description Cut a file/directory from it's source path to the destination path asynchronously (supports stream mode).
         */
        async cut(srcPath, destPath, options = { stream: true }) {
            ...
            if (_isDirSync(srcPath)) {
                ...
                _assert(await _cutFile(srcPath, destPath, opts), `Failed to cut file "${srcPath}" to "${destPath}".`);
            } else {
                _assert(false, "Other types are not currently supported.");
            }
        }
        ...
        renameSync(oldPath, newPath) {
            ...
            _assert(this.isExistSync(oldPath), `The file/directory pointed by the path "${oldPath}" does not exist.`);
            _assert(
         ...
         * @description Bulk operations in order synchronously.
         */
        bulkSync(list) {
        ...
        /**
         * @param {Array} list An task list.
         * @description Bulk operations in order asynchronously.
         */
        async bulk(list) {
            _assert(list instanceof Array, 'The "list" argument must be of type array.');
            for (let i = 0; i < list.length; i++) {
